        ...
        <nav className="hidden items-center gap-5 lg:flex xl:gap-8">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="group inline-flex items-center gap-1 whitespace-nowrap text-base font-semibold leading-7 text-[#292E3E] transition-colors hover:text-[#1E6DEB] xl:text-[18px]"
            >
              {link.label}
              {link.caret ? (
                <ChevronDown
