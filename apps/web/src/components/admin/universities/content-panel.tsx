...
export function ContentPanel({
  ...
  // Ordering only means anything inside a section, so the positions rewritten
  // here are indices within that section's own list, matching how the API
  // hands out the next sortOrder on create.
  async function move(sectionBlocks: ContentBlockRow[], index: number, direction: -1 | 1) {
    const target = index + direction;
    const moved = sectionBlocks[index];
    const displaced = sectionBlocks[target];
    if (!moved || !displaced) return;

    setMovingId(moved.id);
    const base = `/api/admin/universities/${universityId}/content-blocks`;
