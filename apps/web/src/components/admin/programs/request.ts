 ...
 *
 * The admin API answers a rejection with `{ error, field }`, the program
 * DELETE answers a 409 with a `counts` object, and both child collections
 * answer a duplicate with a 409 whose message names database columns. So a
 * failure has to hand back the status and the raw body as well as the message
 * — the form highlights the field, the delete dialog reads the counts, the
