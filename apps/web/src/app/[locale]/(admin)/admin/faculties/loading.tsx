...
/**
 * Mirrors the real page block for block — header, toolbar, six table rows —
 * so the layout does not shift when the query lands.
 */
    ...
    <div className="mx-auto w-full max-w-[1400px] px-4 py-6 md:px-8 md:py-8">
      ...
      <div className="mt-6 flex flex-col gap-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="h-10 w-52 animate-pulse rounded-lg bg-slate-100 sm:ms-auto" />
        </div>
            ...
            {Array.from({ length: 6 }).map((_, index) => (
              <div key={`row-${index}`} className="flex items-center gap-4 px-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="h-3.5 w-52 max-w-full animate-pulse rounded bg-slate-100" />
                  <div className="mt-2 h-3 w-32 animate-pulse rounded bg-slate-100" />
                </div>
                <div className="hidden h-3.5 w-40 animate-pulse rounded bg-slate-100 md:block" />
                <div className="hidden h-3.5 w-24 animate-pulse rounded bg-slate-100 lg:block" />
                <div className="h-3.5 w-10 animate-pulse rounded bg-slate-100" />
                <div className="size-9 shrink-0 animate-pulse rounded-lg bg-slate-100" />
              </div>
            ))}
          </div>
